import pyinputplus as pyip  # Pour les entrées utilisateur avancées
import string  # Manipulation de chaînes
import pyfiglet
import stdiomask  # Masquage des mots de passe

import hashlib  # Fonctions de hachage
import math
import os

from Crypto.Cipher import AES  # Algorithmes de chiffrement
from Crypto.Cipher import Salsa20
from Crypto.Random import get_random_bytes   # Génération aléatoire sécurisée
...
class SymmetricEncryption:
	...
	def encrypt(cls, method='AES'):
		...
		plaintext = str.encode(plaintext)  # Convertit en bytes
      
        # Demande la clé secrète (masquée)

		key = stdiomask.getpass()
		key = str.encode(key)

         # Ajuste la taille de la clé
		if(method=='AES'):
			if(len(key)<16):
